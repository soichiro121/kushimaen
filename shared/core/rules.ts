 ...
 *
 * Lives in `shared/` because the API routes read it too. Only relative imports in
 * here: this file is bundled both by Vite (with its aliases) and by the serverless
 * function builder (without them).
 *
 * The raw JSON in `shared/game-rules/` is the single source of truth and is read by
 * the API routes as well. Bump the version (a new `vN.json` and a new import here)
 * whenever a change would move existing scores, so the two generations stay
