 ...
 * Pure score formulas.
 *
 * MIRRORED IN PHP: `backend/src/Service/ScoreCalculator.php`.
 * Both implementations read the same constants from `shared/game-rules/v3.json`
 * and are pinned by the golden fixtures in `shared/game-rules/score-fixtures.json`.
 *
 * Invariants:
...
/** Metrics travel over the wire as a flat numeric map, so PHP reads them trivially. */
