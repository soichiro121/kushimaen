 ...
 *
 * MIRRORED IN PHP: `backend/src/Validation/StageResultValidator.php` +
 * `backend/src/Service/ScoreCalculator.php`. The authoritative check is the PHP one -
 * this copy exists so LOCAL MODE behaves like production and so the rules are unit
 * tested on the client too. Both read their bounds from `shared/game-rules/vN.json`,
 * so the numbers cannot drift even though the code is written twice.
 */
import { RULES, type StageId } from '@/config/rules';
import { computeStageScore, maxPlausibleStageScore } from '@/game/core/scoring';
...
function metricIssues(stageId: StageId, metrics: Record<string, number>): ValidationIssue[] {
  ...
  switch (stageId) {
    ...
    case 'teacher': {
      ...
      // consistency instead. MIRRORS `teacherIssues` in the PHP validator.
